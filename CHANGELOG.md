...
本文件记录 news-reader 的版本更新历史。项目介绍、运行方式和数据边界见 [README.md](README.md)。

## What's Changed

### 2026-09-01 — v2.1.4.5 fix: 悬浮 Chat 红绿灯控制与操作区布局
- 悬浮 Chat 顶部控制改为 macOS 风格无文字圆点，按从左到右固定为红色收起、黄色放大/还原、绿色新会话；原有按钮 ID、键盘焦点、aria-label/title、禁用条件与点击功能保持不变。
- 底部归档、停止生成（运行中出现）和发送操作整体右对齐，发送始终位于最右侧；不改变操作出现条件、会话语义或后端行为。
- 普通主题保留现有实底与阴影，墨水屏继续使用白底实线边框与可辨识焦点/禁用态；同步页面版本与 CSS/JS cache-bust。

### 2026-08-31 — v2.1.4.4 fix: 修复悬浮 Chat 长内容横向溢出
- 悬浮 Chat 的消息正文、输入前引用和消息内引用增加连续英文、长 URL、自动链接及长代码行的断行规则，避免内容撑宽消息区和悬浮窗。
- 保留现有 `pre-wrap` 换行/空格语义、长引用自身的纵向滚动，以及桌面、手机和墨水屏的既有布局与外观；不使用全局横向裁切掩盖内容宽度问题。
- 本版本仅更新前端样式与页面版本/cache-bust；v2.1.4.3 的悬浮 Agent 归档按钮、归档防重、成功同步和失败保留状态均保持不变。

### 2026-08-31 — v2.1.4.3 fix: 恢复悬浮 Agent 归档到“我的想法”
- 恢复当前悬浮新闻研究 Agent 输入框下的“归档”按钮，与发送、停止等控件继续靠左排列。
- 仅在当前会话存在非空 Agent 回答且没有发送、归档或后台研究任务时启用；归档期间输入与发送控件同步禁用，避免重复请求。
- 复用现有 Pi 归档接口，将当前可见问答压缩为不超过 200 字的中文结论并追加到本篇“我的想法”；成功后立即同步正文想法区、中栏想法状态与预览。
- 失败时保留当前 Agent 会话和原有想法，不新增归档历史或存储；同步版本号 v2.1.4.3（title、顶栏、CSS/JS cache-bust、移动端更多面板、README）。

### 2026-08-30 — v2.1.4.2 fix: 调整悬浮 Agent 界面层级
- 悬浮 Agent 不再重复展示正文中的中文要点；中文要点仍保留在 Agent 所需的新闻上下文中。
- 输入框下方的发送、停止等操作控件统一靠左排列，原有功能与出现条件不变。
- 普通主题使用独立实色悬浮面板和更明确的外阴影；墨水屏主题保持黑白实底、实线边框且不使用模糊阴影。
- 桌面端首次展开时悬浮窗默认高度为右栏可用内容区的 2/3，可从顶部拖拽调整并保存到当前浏览器；放大后还原到保存高度，手机端不启用拖拽。
- 同步版本号 v2.1.4.2（title、顶栏、CSS/JS cache-bust、移动端更多面板、README）。
...
- 顶栏与移动端“更多”的外观选择新增可持久化的“墨水屏”：白底黑字、深灰辅助字、实色不透明面板、1–2px 清晰边框；关键组件最终样式无渐变、透明叠层、模糊、阴影、动画与缩放；主题在 head 内联脚本中于 CSS 首帧前恢复，不闪普通主题。
- 触控优先：原依赖 hover 显隐的行内操作在墨水屏下常显，常用触控目标提升到约 44px，保留键盘焦点态；辅助信息不再用低透明度弱化，禁用态改虚线边框表达。
- 状态脱离颜色：未读实心点、选中粗描边、重要 ★、看多/看空 ↑/↓ 前缀；四色高亮分别以实线/虚线/双线/点线下划线区分，批注按钮保留形状语义；正文至少 16px、行高约 1.65。支持 `?theme=eink` 会话级直达，不静默修改长期偏好。
- 仅前端主题层变更，不改 API、数据库、新闻状态、轮询与自动已读；浅色/深色/跟随系统行为不变。同步版本号 v2.1.3.0（title、顶栏、CSS/JS cache-bust、移动端更多面板、README）。
...
- 同步版本号 v2.1.2.4（title、顶栏、CSS/JS cache-bust、移动端更多面板、README）。

### 2026-08-10 — v2.1.2.3 fix: 推文正文高亮与浮窗关闭
- 未翻译 Twitter 详情主正文新增 `twitter_detail` 高亮面，严格以该推文当前 `article_details.content` 为权威文本；已翻译推文继续复用 `ai_zh`，普通新闻英文原文折叠区不在本轮范围。
- 原始推文高亮完整复用四色、批注、正文变更重定位与 orphan 防守，并通过迁移无损保留既有三类高亮记录。
- 高亮与批注浮窗支持外部空白点击、正文/页面滚动和移动端滑动关闭；浮层内部与 textarea 内滚动不误关，外部关闭编辑态按取消处理并隔离晚到请求。
- 同步版本号 v2.1.2.3（title、顶栏、CSS/JS cache-bust、移动端更多面板、README）。

### 2026-08-09 — v2.1.2.2 feat: 高亮批注与尾端标注
- 每条高亮支持一条最多 2000 字的纯文本批注，旧库迁移默认空批注；提供独立读取、保存、编辑与清除 API，并校验新闻 URL 归属。
- 正文、中文要点与末段总结的可见高亮尾端增加不参与正文 offset 的批注按钮，区分添加态与已有批注态。
- 新增紧凑批注编辑浮层，支持新建、修改、清除与取消；轮询重绘时保留同新闻的草稿与焦点，失败时保留原批注和输入。
- 改色、重定位和 orphan 处理保留批注，高亮删除时批注随之删除；同步版本号 v2.1.2.2（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-08-09 — v2.1.2.1 feat: 扩展中文正文高亮交互
- 拖选中文正文或中文要点时直接显示“高亮”按钮，保留紧凑浮层外框供后续操作扩展。
- 高亮记录支持黄色、绿色、蓝色、粉色四种颜色；正文与中文要点均以安全的纯文本节点和 `<mark>` 渲染。
- 终验返修：颜色改为浮层内独立小按钮；已有高亮可直接改色，取消态不再显示旧提示文案。
- 终验返修：高亮浮层收窄为纯四色按钮；中文要点最后的独立总结字段使用 `ai_conclusion_zh` 单独保存、重定位与渲染，避免跨字段拼接 offset。
- 独立扩展高亮覆盖层的数据模型与正文重定位链路，保留刷新/重开持久化、稳定态门禁、重叠校验和待重新定位提示。
- 同步版本号 v2.1.2.1（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-08-07 — v2.1.2.0 feat: 中文正文单色选区高亮
- 中文正文稳定生成后支持选择文字即时创建黄色高亮，刷新/重新打开详情仍可恢复；点击已有高亮后通过“取消高亮”动作删除。
- 新增 `article_highlights` SQLite 持久化与高亮查询、创建、删除 API；严格校验正文 hash、选区偏移、选中文本、上下文与重叠，正文变更时仅对唯一且上下文匹配的选区自动重新定位。
- 使用纯文本节点与 `<mark>` 渲染，不修改原始正文或 AI 正文；无法唯一定位的记录显示待重新定位数量。
- 同步版本号 v2.1.2.0（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-08-02 — v2.1.1.3 fix: 信源设置即时保存
- 设置页“新闻流信源显示”移除独立“保存信源设置”按钮，关闭或恢复二级信源时立即写入 `app_settings.json`。
- 恢复二级信源不再弹出确认框；操作后立即刷新新闻流过滤、来源统计与左栏角标。
- 同步版本号 v2.1.1.3（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-31 — v2.1.1.2 fix: 三项交互修复
- 修复已读新闻标题样式覆盖重要/看多/看空/分歧语义色的问题：已读只弱化普通标题，重要标题继续保持黄色。
- 跟踪集合隐藏中栏通用“标为已读”和“刷新”按钮，仅保留跟踪专属“新建跟踪/默认参数”。
- 板块管理新增上移/下移排序控件与 `/api/market-tags/reorder` 原子排序接口，完整校验后连续写入 `sort_order`，创建追加末尾，删除/合并后归一化顺序。
- 同步版本号 v2.1.1.2（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-31 — v2.1.1.1 improve: 左栏集合角标
- 左栏新闻流、稍后再看、复盘集合新增角标：新闻流按当前有效未读计数并应用二级信源隐藏，稍后再看按独立队列未完成数计数，复盘按已到期待复盘链计数。
- 新增 `/api/nav-summary` 一次返回三类导航计数，首屏、集合刷新、阅读状态、稍后队列、复盘完成/再次跟踪和二级信源设置变更后刷新；0 值隐藏并补充 aria-label。
- 同步版本号 v2.1.1.1（title、顶栏、CSS/JS cache-bust、移动端更多面板）。
...
- 修正 README 中 host/port、Tailscale 启动脚本与 DeepSeek Keychain service 的事实口径；同步版本号 v2.1.0.15（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-21 — v2.1.0.14 fix: 保留右栏轮询刷新期间的临时交互态
- 同一新闻的详情轮询与行状态轮询改走非破坏式渲染，后台刷新仍更新正文、进度、按钮与提醒/板块卡片，但不关闭正在使用的写想法、看多/看空、提醒、加入跟踪编辑器。
- 写想法编辑器在刷新期间保留未保存草稿、焦点与 selection；板块 picker 保留展开方向与候选项；提醒/跟踪编辑器保留已填/已选值。
- 切换新闻、关闭详情、主动取消或保存成功仍清理临时态，避免串稿；同步版本号 v2.1.0.14（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-19 — v2.1.0.13 improve: PC 中栏新闻行键盘状态动作与右栏焦点切换

- 桌面中栏键盘模式扩展 `←/→` 状态动作：`←` 复用重要切换，`→` 复用稍后再看切换；长按 repeat 事件只拦截不重复翻转，状态写入沿用既有 optimistic UI、失败回滚、重试反馈与轮询链路。
- `→` 仅在当前行真实存在 `.btn-read-later` 时生效，Bloomberg video 等不支持稍后再看的行保持 no-op，不绕过产品限制。
- `Enter/Return` 会立即提交当前 120ms 尾随详情打开，只触发最终选中项一次详情抓取/轮询/checkpoint，并把焦点移入右栏正文滚动区。
- 右栏通过 Enter 进入 keyboard focus mode 后，任意焦点目标按 `Esc` 都会保留详情与草稿并回到原中栏选中行；原行消失、切换集合、重置列表、关闭详情或缩到非桌面宽度时安全清理模式与焦点视觉。
- 同步版本号 v2.1.0.13（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-17 — v2.1.0.11 design: 日报元数据的响应式分组

- 日报列表与详情保留执行模式、使用文件和执行时间等完整元数据，并按字段组渲染，避免移动端在文件名中间断行；移动端隐藏装饰性分隔点，桌面端保留原有节奏。
- 保持日报预留工具栏、正文层级和所有直接可见操作不变。

### 2026-07-17 — v2.1.0.10 design: 延续已确认的视觉基线

- 版本标识与 CSS/JS cache-bust 统一升级到 v2.1.0.10；继续以 v2.1.0.9 的稳定 UI 作为基线，不引入全局 Liquid Glass 重构或隐藏预留工具栏。

### 2026-07-16 — v2.1.0.9 design: 邻近反馈与失败恢复

- 新增可复用的行内反馈语法：pending、ready、failed 状态只在受影响的新闻行、详情、编辑器或证据卡内出现，并以文字说明状态，不依赖颜色记忆。
- 重要、收藏和稍后阅读保存失败时会恢复原状态，在当前新闻行及已打开详情中说明哪项操作未保存，并提供直接重试；自动已读不生成重复告警。
- 复盘创建、记录进展、修正判断、完成复盘、继续观察和再次跟踪的校验与失败提示移回当前右栏表单；提交期间阻止重复操作，失败后保留全部输入。
- 复盘详情加载失败新增就近重试，证据删除失败保留在证据卡内；跟踪主题、规则草稿和默认参数的 pending / success / failure 状态统一进入当前编辑器。
- 新闻想法、板块想法、独立想法、提醒、加入跟踪、板块方向和详情重试同样使用邻近反馈；写入成功但后续刷新失败时明确提示“已保存”，并阻止重复创建。
- 同步版本号 v2.1.0.9（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-16 — v2.1.0.8 design: 新闻行扫读层级与状态分区

- 主新闻行把紧邻的“来源 · 时间”元数据组与三项个人动作拆为稳定的两列顶部结构；重要、稍后阅读、收藏继续全部直接可见，来源与时间不再和想法、提醒共同争抢空间。
- 来源和时间保留独立文本层级但相邻显示；想法预览移入轻量注释区，提醒和市场标签进入摘要后的上下文区，保持“来源与时间—标题—摘要—个人上下文”的固定扫读顺序。
- 移除逐行重复的后台任务文字标签，由稍后阅读按钮统一表达详情任务状态：抓取中保持琥珀色、就绪保持绿色，抓取失败新增红色底色与描边，并通过提示和无障碍标签说明准确状态。
- 已读点改为保留占位的透明状态，避免已读/未读切换时来源起点跳动；补齐“已读且选中”的选中色调，以及键盘进入行内动作时的显现状态。
- 新闻行主间距对齐既有 spacing token，并使用主文章专用类隔离改动，避免影响提醒、想法、市场、复盘、日报和跟踪主题列表。
- 同步版本号 v2.1.0.8（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-16 — v2.1.0.7 design: 导航信息架构与紧凑宽度矩阵

- 桌面左栏由“阅读 7 项同级”重组为“阅读、个人队列、研究、市场”四个语义组；搜索、新闻流、日报、稍后再看、重要、提醒、收藏、想法、复盘、跟踪、板块全部保留直接可见。
- 导航标签与提醒计数分列显示，当前集合统一写入 `aria-current="page"`；来源筛选显示“全部来源”总条数，并为当前来源补充 `aria-pressed` 状态。
- `1180px` 以下提前切换单栏工作区，四组导航以紧凑矩阵排列，修复原三栏在 `981–1137px` 的横向溢出；复盘集合不再显示无效来源筛选。
- 移动端继续保留三项底栏与上下文面板，但面板分组与桌面一致；进入次级集合后，底栏“更多”会显示实际集合名称。
- 同步版本号 v2.1.0.7（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-16 — v2.1.0.6 design: 跟踪参数分区与响应式对齐

- 跟踪主题编辑器按“主题定义、关键词规则、收录门槛、字段倍率、规则得分、增量范围与状态”建立稳定分区；全部既有参数继续常驻显示，不折叠、不隐藏，也不加入“更多”菜单。
- 关键词输入加入持久标签与就近说明，并让 `rows=2/3` 恢复真实高度差；数值参数改为标签—数值对齐网格，桌面双列比较，`640px` 以下单列排列。
- 默认参数页复用相同的门槛、倍率与得分结构；保存操作固定在编辑器滚动区底部，长表单中无需寻找提交入口；重新打开新建、编辑或默认参数时从表单顶部开始。
- 同步版本号 v2.1.0.6（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-16 — v2.1.0.5 design: 中栏工具栏任务分组与移动端滚动反馈

- 中栏工具栏按“当前视图筛选与创建、阅读与排序、批量与维护”建立稳定分组；提醒、想法、复盘、跟踪、板块与稍后阅读的既有控件全部保留直接可达。
- 桌面端使用组内紧凑、组间留白和内容组换行建立层级；统一中栏面板、工具栏和筛选按钮的间距 token，移除 `13px/11px/7px` 等尺度外值。
- 移动端保留单行横向滚动，加入随滚动位置变化的左右渐隐；复盘结果筛选改为同一滚动序列，切换集合时工具栏自动回到起点。
- 同步版本号 v2.1.0.5（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-16 — v2.1.0.4 design: 详情工具栏信息分组与移动端溢出提示

- 文章详情工具栏按“来源与恢复、研究与想法、提醒与个人状态、板块方向”分为四组；所有既有操作保持直接可达，不加入“更多”菜单，也不改变按钮事件与后端语义。
- 桌面端使用组内紧凑、组间留白和整组换行建立层级；统一详情面板与工具栏的间距节奏，避免动作区靠弹性空白分裂。
- 移动端保留紧凑横向滚动，加入随滚动位置动态变化的左右渐隐提示，并在末端预留空间，避免末个按钮被渐隐遮盖。
- 同步版本号 v2.1.0.4（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-15 — v2.1.0.3 fix: 复盘右栏单滚动区

- 复盘右栏保留固定 `detail-actions detail-toolbar` 工具栏，编辑表单移入正文滚动区顶部，避免“提前复盘/完成复盘”时顶部编辑区与正文形成上下两个滚动区并视觉重叠。
- 复盘正文、时间线与编辑表单统一由 `detailReviewScrollArea` 一个容器滚动；移除编辑表单自身高度限制和内部滚动，长表单按钮随同一滚动区可达。
- 保留 v2.1.0.2 的 full-bleed 修复基准：工具栏不进入滚动容器，`detailReviewBody` 不新增中间 overflow 裁切层，避免负边距工具栏边缘再次被裁。
- 同步版本号 v2.1.0.3（title、顶栏、CSS/JS cache-bust、移动端更多面板）。

### 2026-07-14 — v2.1.0.2 fix: 复盘工具栏结构、顶部编辑区与结果筛选

- 复盘集合隐藏与其无关的“已读”和“刷新索引”动作，普通新闻流、重要、稍后阅读等集合保持原有能力。
- 复盘右栏改为非滚动顶部工具栏 + 顶部编辑槽 + 独立正文/时间线滚动区，移除复盘详情外层对工具栏的 `overflow` 裁切，修复工具栏背景显示为内缩暗条的问题；工具栏恢复复用统一的 `detail-actions detail-toolbar` 结构，并在桌面端与中栏工具栏等高、铺满面板边缘、保持直角与底部分隔线。
- “记录进展”“修正判断”“提前/开始复盘”“再次跟踪”的表单统一紧随工具栏显示，仅保留一个编辑面板，首个输入项自动聚焦；取消后恢复详情阅读态。
- “已完成”复盘主筛选下新增“结果”子栏：全部、成立、未成立、不可判断。结果筛选复用既有 `result` 语义，并由 `/api/reviews?result=` 在服务端筛选，保证分页/继续加载不会遗漏未加载结果；不展示或迁移历史“部分成立”。
- 同步版本号 v2.1.0.2（title、顶栏、CSS/JS cache-bust、移动端更多面板）。
- **影响**：`app.py` 的复盘列表接口新增可选 `result` 查询参数，部署后需重启 Flask；刷新页面以加载新的前端静态资源。
...
- **文件**
  - *static/index.html（~）*、*static/app.js（~）*、*app.py（~）*、*tests/test_api.py（+）*、*README.md（+）*
    - `detailTrendIdeaBody` 与 `detailStandaloneIdeaBody` 查看态顶部统一为 `detail-actions detail-toolbar`
    - 现有「编辑板块想法 / 删除板块想法」与「编辑 / 删除」按钮移到标题之前的工具栏，文字按钮靠左排列
    - 结构顺序统一为：顶部工具栏 → 标题 → 元信息 → 全文卡片 / 编辑器
    - 板块想法编辑态与「新板块想法」右栏同构：日期、板块、方向、正文四项及保存/取消按钮均出现，字段顺序、控件样式、文案复用新建界面
    - `PATCH /api/market-trends/note/<id>` 扩展为可同步更新日期、板块、方向、正文；保持原有只传 `note` 的向后兼容；不新建 schema/数据库/业务模型
    - 编辑后若日期/板块/方向发生迁移，前端会重新加载想法列表并定位到迁移后的记录，旧位置/计数正确刷新
    - 独立想法编辑态本轮不扩字段，仅修正顶部按钮靠左
    - 保存 / 取消继续留在各自编辑器内部；新建板块想法、新建独立想法表单不增加空工具栏
    - 保持按钮 ID、事件绑定、确认文案不变；不改为新图标，不扩展新操作（板块想法迁移复用现有接口）
    - 同步版本号 v2.0.3.2（title、顶栏、CSS/JS cache-bust、移动端更多面板）
- **影响**：涉及 `app.py` PATCH 接口与前端状态同步，拉取后需重启 Flask 并刷新页面。
...
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*、*static/app.js（+）*、*README.md（+）*
    - 修正桌面端右栏正文阅读宽度：不再固定停留在 `72ch` 左右，而是可随右栏可用宽度增长
    - 右栏标题、meta、摘要、AI 框、正文与英文原文统一复用新的阅读宽度上限，减少大屏右侧大块空白
    - 保留移动端正文排版与窄屏无横向溢出的既有修复口径
    - 顶栏版本号、页面 `<title>`、静态资源版本参数与移动端更多面板版本文案同步更新到 `v2.0.1.4`
- **影响**：本轮只调整右栏前端排版宽度与版本说明，不修改后端 API、数据库 schema、导入、AI、提醒或跟踪逻辑；拉取后刷新页面即可。

### 2026-07-01 — v2.0.1.3 fix: 中间栏顶部工具栏按钮优化
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*、*static/app.js（+）*
    - 提醒集合顶部把 `进行中 / 已完成 / 全部` 收入中间栏顶部工具栏，不再单独占一行
    - 想法集合顶部把 `全部 / 新闻想法 / 趋势想法` 收入中间栏顶部工具栏，保持原筛选语义不变
    - 板块集合把板块下拉、内容筛选与 `添加独立想法` 等操作收入中间栏顶部工具栏，减少控件分裂
    - 顶栏版本号、页面 `<title>`、移动端更多面板版本文案与静态资源版本参数同步更新到 `v2.0.1.3`
- **影响**：本轮只调整中间栏顶部工具栏的前端结构与样式，不修改后端 API、数据库 schema、板块/提醒/想法功能语义；拉取后刷新页面即可。

### 2026-07-01 — v2.0.1.2 improve: 桌面端正文同步手机端排版
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*、*static/app.js（+）*、*tests/test_api.py（+）*
    - 桌面端详情正文、中文要点、结论和英文原文同步使用手机端已验证的普通换行、中文 loose 断行与更稳定的长词换行策略
    - 桌面端中文要点、结论和正文启用两端排版，减少宽栏阅读时的右侧行尾参差
    - 补充清理测试期导入来源提示：刷新索引提示不再暴露 sidecar/fallback/Markdown 分类，详情页不再显示导入路径
    - 日期 header 改为圆角胶囊样式；全局隐藏滚动条但保留滚动能力
    - 顶栏版本号、页面 `<title>` 与静态资源版本参数同步更新到 `v2.0.1.2`
- **影响**：本轮只调整详情页/列表前端显示与静态检测测试，不修改后端 API、数据库 schema、导入、AI、提醒或跟踪逻辑；拉取后刷新页面即可。

### 2026-07-01 — v2.0.1.1 fix: 手机端正文排版
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*
    - 手机端详情正文取消桌面 `text-wrap: pretty` 的主动重排行为，改用更适合窄屏中文阅读的普通换行与宽文本面
    - 中文要点和正文在手机端启用更自然的中文断行与两端排版，减少每行右侧大面积留白
    - 顶栏版本号、页面 `<title>` 与静态资源版本参数同步更新到 `v2.0.1.1`
- **影响**：本轮只调整手机端详情页前端排版，不修改后端 API、数据库 schema、导入、AI、提醒或跟踪逻辑；拉取后刷新页面即可。

### 2026-06-30 — v2.0.1.0 ui: 移动端三入口重构
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*、*static/app.js（+）*
    - 移动端取消桌面式顶栏和额外 context header，中栏去外框并贴合底部栏，首屏优先显示新闻列表
    - 底部主导航调整为 `新闻 / 稍后 / 更多` 三个高频入口，`更多` 分组承载搜索、重要、收藏、提醒、想法、跟踪、板块和系统入口
    - 来源筛选改为列表工具条最左侧入口，打开现有 bottom sheet，保留未读/排序/刷新等前端行为
    - 顶栏版本号、页面 `<title>` 与静态资源版本参数同步更新到 `v2.0.1.0`
- **影响**：本轮只调整移动端前端结构与入口组织，不修改后端 API、数据库 schema 或导入/AI/提醒/跟踪逻辑；拉取后刷新页面即可。

### 2026-06-30 — v2.0.0.0 ui: 电脑端 UI Phase 1 视觉重构
- **文件**
  - *static/index.html（+）*、*static/style.css（+）*、*static/app.js（+）*
    - 重构电脑端阅读工作台视觉层级：顶栏显示选项、左侧集合与来源筛选、中栏新闻列表、右侧详情阅读面板
    - 优化来源图标识别、来源筛选 badge、新闻标题截断、详情提醒卡片展开收起与滚动条主题色
    - 顶栏版本号、页面 `<title>` 与静态资源版本参数同步更新到 `v2.0.0.0`
  - *tests/test_api.py（+）*
...
- **文件**
  - *static/app.js（+）*
    - 将新闻流列表加载抽成可指定页码的单次加载路径，保留现有排序、筛选、日期分组与自动已读逻辑
    - `回到上次阅读` 从“先加载第 1 页，再逐页补到目标页”改为“定位后直接请求目标页”，避免在 10000+ 新闻里顺序请求数百页
    - 命中锚点后仍会高亮并自动滚动到目标行；桌面端继续自动打开右栏详情
  - *static/index.html（+）*
    - 顶栏版本号、页面 `<title>` 与静态资源版本参数同步更新到 `v1.9.9.5`
- **影响**：在新闻流 `全部显示` 场景下，点击“回到上次阅读”不再因为逐页补加载而明显变慢；当前版本仍按方案 A，只保证快速直达锚点所在页，不预加载锚点之前的页面。
