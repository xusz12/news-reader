...
### 2026-07-19 — v2.1.0.12 improve: PC 中栏新闻行键盘导航

- 桌面三栏布局中，鼠标选中真实新闻行后进入中栏键盘模式；`↑/↓` 只在相邻新闻行间移动，复用既有详情打开链路，并在边界处停止不循环。
- 末条新闻且还有更多数据时，`↓` 只触发一次既有增量加载，成功后选择新增的下一条新闻；加载失败或正在加载时保留当前选择，避免并发翻页。
- 详情抓取、轮询和阅读 checkpoint 对快速连续移动做约 120ms 尾随合并；行选中态、右栏基础内容和滚动位置即时同步。
- 搜索框、右栏正文/编辑器、行内按钮、IME 与组合键不被箭头劫持；点击中栏外或按 Escape 退出键盘模式但保留当前详情。
- 同步版本号 v2.1.0.12（title、顶栏、CSS/JS cache-bust、移动端更多面板）。
