...
### 2026-06-15 — improve: 新增 v2.1.1 设置页分区侧栏与推荐关键词库分栏
- **文件**
  - *static/index.html（+）*、*static/app.js（+）*、*static/style.css（+）*
    - 设置页从纵向堆叠改为“左侧细窄分区导航 + 右侧单板块内容”，将 `服务管理 / 模型管理 / 推荐关键词库 / Release Notes` 拆成独立 section，点击侧栏后同一时间只显示一个板块
    - 移动端设置页导航退化为顶部可横滚 tab，不再回退到长列表堆叠
    - 推荐关键词库板块新增 `启用中 / 已禁用 / 待审核 candidate` 三态 tab，并显示数量；存在待审核 candidate 时会做更醒目的提示
    - `启用中 / 已禁用` 关键词列表改为紧凑多列关键词格，一行可显示多个 keyword；candidate 继续保留详细 reason/sample/审核动作卡片
- **影响**：设置页浏览成本显著下降；推荐关键词库从“信息全铺开”改为“按状态切页查看”，更适合持续人工审核和维护。
